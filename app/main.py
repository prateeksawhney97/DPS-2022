...
@app.route("/api/predictalcohol", methods=['POST'])
def apipredictalcohol():
    comment_data = request.get_json()
    year = comment_data['year']
    month = comment_data['month']
    print(comment_data)
    if type(year) != int or month not in range(1, 13):
        return {"Error": "Year & Month must be valid numbers!"}, 400
    else:
        df_meta = pd.read_csv('210619monatszahlenjuni2021monatszahlen2106verkehrsunfaelle.csv')
        df_meta = df_meta[df_meta.MONAT != 'Summe'] # Drop unnecessary rows
        df_meta = df_meta.sort_values(by ='MONAT' , ascending=True) # Sort values by time
        ...
        if user_month==1:
            ...
            result_alcohol=P12
        prediction = result_alcohol
        return {'prediction': prediction}

@app.route("/api/predicttraffic", methods=['POST'])
def apipredicttraffic():
    comment_data = request.get_json()
    year = comment_data['year']
    month = comment_data['month']
    print(comment_data)
    if type(year) != int or month not in range(1, 13):
        ...
        if user_month==1:
            ...
            result_traffic=P12
        prediction = result_traffic
        return {'prediction': prediction}

@app.route("/api/predictescape", methods=['POST'])
def apipredictescape():
    comment_data = request.get_json()
    year = comment_data['year']
    month = comment_data['month']
    print(comment_data)
